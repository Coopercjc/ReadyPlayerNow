...
public class PlayerMovement : MonoBehaviour
{
    ...
    void HandleAnimations(float HorizontalMovement, float VerticleMovement)
    {
        if (isMultiDirectional)
        {
            ...
            HandlePlayerOrientation2D(HorizontalMovement, VerticleMovement);
        }
        else
        {
    ...
    void HandleMovement(float HorizontalMovement, float VerticleMovement)
    {
        ...
        if (!canJump && HorizontalMovement == 0)
        {
            Vector3 inputVector = new Vector3(0, VerticleMovement, 0).normalized;
            transform.Translate(inputVector * ScaledSpeed);
        }
        else if (!canJump && VerticleMovement == 0) {
            Vector3 inputVector = new Vector3(HorizontalMovement, 0, 0).normalized;
            transform.Translate(inputVector * ScaledSpeed);
        }
        /*else
        {
            //rb.velocity = new Vector2(HorizontalMovement * PlayerMovementSpeed, rb.velocity.y);
            Vector3 inputVector = new Vector3(0, 0, 0).normalized;
            HandlePlayerOrientation2D(HorizontalMovement, VerticleMovement);
            transform.Translate(inputVector * ScaledSpeed);
        }*/
    }
    ...
    void HandlePlayerOrientation2D(float HorizontalVelocity, float VerticalVelocity)
    {
        if (HorizontalVelocity < 0)
        {
            PlayerAnimator.SetFloat("IdleX", -1);
            PlayerAnimator.SetFloat("IdleY", 0);
        }
        else if (HorizontalVelocity > 0) {
            PlayerAnimator.SetFloat("IdleX", 1);
            PlayerAnimator.SetFloat("IdleY", 0);
            ...
            PlayerAnimator.SetFloat("IdleY", -1);
            PlayerAnimator.SetFloat("IdleX", 0);
        }
        else if (VerticalVelocity > 0) {
            PlayerAnimator.SetFloat("IdleY", 1);
            PlayerAnimator.SetFloat("IdleX", 0);
